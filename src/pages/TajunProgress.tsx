...
import { Link } from "react-router-dom";
import { Calendar, Construction, MapPin, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
...
const TajunProgress = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background">
      <Navigation />
      {/* Hero */}
      <section className="relative h-[70vh] min-h-[500px] w-full overflow-hidden pt-20">
        <img
          src={tajunHero}
          alt="Tajun Heights"
          className="absolute inset-0 h-full w-full object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/40 to-black/80" />
        <div className="container relative z-10 mx-auto flex h-full flex-col justify-end px-6 py-8 md:px-12">
          <div className="max-w-3xl text-white">
            <Badge className="mb-4 bg-accent text-accent-foreground">
              Ongoing Project · Under Construction
            </Badge>
      ...
      {/* Stats */}
      <section className="border-b border-border bg-secondary/30 py-10">
        <div className="container mx-auto grid grid-cols-3 gap-3 px-6 md:gap-6 md:px-12">
          {[
            { label: "Total Floors", value: "15+", icon: Construction },
            { label: "Project Start", value: "2022", icon: Calendar },
            { label: "Location", value: "Wadala", icon: MapPin },
            ...
            <div
              key={i}
              className="rounded-2xl border border-border bg-card p-4 text-center shadow-sm md:p-6"
            >
              <s.icon className="mx-auto mb-2 h-6 w-6 text-primary" />
              <p className="text-lg font-bold sm:text-2xl md:text-3xl">{s.value}</p>
              <p className="text-xs uppercase tracking-wider text-muted-foreground">
                {s.label}
              </p>
            </div>
          ...
          <div className="relative mx-auto max-w-5xl">
            ...
            {timeline.map((item, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`relative mb-12 flex flex-col gap-6 pl-12 md:mb-16 md:pl-0 md:flex-row md:items-center ${
                    isLeft ? "md:flex-row" : "md:flex-row-reverse"
                  }`}
                >
                  {/* Dot */}
                  ...
                  <div className="w-full md:w-1/2 md:px-6">
                    <button
                      ...
                      className="group block w-full overflow-hidden rounded-2xl border border-border shadow-elegant"
                  ...
                  {/* Text */}
                  <div className="w-full md:w-1/2 md:px-6">
                    <Badge className={`mb-3 ${statusStyles[item.status]}`}>
                      {item.status}
                    </Badge>
                    <p className="mb-2 text-sm font-semibold uppercase tracking-widest text-primary">
                      {item.date}
      ...
      {lightbox !== null && (
        ...
        >
          ...
          <div className="max-h-[90vh] max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <img
              src={timeline[lightbox].image}
              alt={timeline[lightbox].title}
            ...
            <div className="mt-4 text-center text-white">
              ...
              <h3 className="mt-1 text-xl font-bold">{timeline[lightbox].title}</h3>
            </div>
          </div>
        </div>
      )}
